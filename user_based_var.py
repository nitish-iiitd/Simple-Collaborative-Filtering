...
def pearson_def_var(items,x, y):
    ...
    for idx in range(n):
        xdiff = x[idx] - avg_x
        ydiff = y[idx] - avg_y
        diffprod += xdiff * ydiff * var_item[items[idx]]
        xdiff2 += xdiff * xdiff
        ydiff2 += ydiff * ydiff
        var_sum += var_item[items[idx]]
    if xdiff2 == 0 or ydiff2 == 0: # Correlation undefined, here we take it as -1
        return -1
    return (diffprod / math.sqrt(xdiff2 * ydiff2))/var_sum
...
var_item = [0 for x in range(item+1)] ## for storing the variance of item's ratings
...
for ii in range(1,item+1):
	currlist = []
	for jj in range(1,user+1):
		if table[jj][ii] != 0:
			currlist.append(table[jj][ii])
	var_item[ii] = np.var(currlist)
...
for u1 in range(1,user+1):
	for u2 in range(u1+1,user+1):
		...
		#print user1,user2
		curr_sim = pearson_def_var(items,user1,user2)
...
print user_sim
pickle.dump( user_sim, open( "u5_var.p", "wb" ) )


##### Testing and finding NMAE #######
user_sim = pickle.load( open( "u5_var.p", "rb" ) )
totalerror = 0
testsetlen = len(u1t)
for line in u1t:
	splitted = line.split()
